...
def run_snarc():
    ...
    def run_snarc_trial(cnt, trial):
        ...
        target.present()
        lsl_stream.push_sample([f"Stim {digit}"])
        btn, rt = exp.keyboard.wait([constants.K_LEFT, constants.K_RIGHT],
                                    duration=1000)
        if btn is not None:
            lsl_stream.push_sample([f"Resp {btn}"])
    ...
    for block in exp.blocks:
        ...
        for cnt in range(0, no_training_trials):
            ...
            run_snarc_trial(-1 * (1 + cnt), trial) #training trails has negative trial numbers
        # Show instruction screen
